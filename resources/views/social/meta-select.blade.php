...
<section class="section">
    ...
    <p class="lead">以下帳號來自你管理的 Facebook Page。請選擇要加入 KOLD 的 Instagram Business／Creator 帳號。</p>
    ...
    <form class="panel" method="POST" action="{{ route('social.meta.store') }}">
        ...
        @foreach ($candidates as $index => $candidate)
            ...
            <label class="profile-row" style="display:grid;grid-template-columns:auto 1fr;gap:.8rem;align-items:start;cursor:pointer">
                <input
                    type="checkbox"
                    name="accounts[]"
                    value="{{ $candidate['key'] }}"
